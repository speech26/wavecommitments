...
fn test_batch_proof_verify(datas: Vec<F>, filename: String) {
    ...
    writeln!(
        file,
        "{:<5} {:15} {:<15} {:<15} {:<15}",
        "width", "build tree", "batch proof", "batch verify", "total"
    )
    ...
    let b =
        VerkleTree::batch_proof_verify(root, proof.clone(), width, indices, depth, datas_verify);
...
fn test_batch_point_proof_verify(datas: Vec<Vec<u8>>, filename: String) {
    ...
    let startverify = Instant::now();
    let b = VerkleTree_point::batch_proof_verify(
        root,
        proof.clone(),
        width,
        indices,
