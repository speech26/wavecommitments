...
impl VerkleTree {
    ...
    */
    ...
    fn find_proof_node(
        ...
        if node.children.is_some() {
            for ind in indices_to_proof {
                let point_1 =
                    &node.children.clone().expect("this child was actually None")[ind].commitment;
        ...
        let proof: Result<G1Affine, ProofError> =
            self.kzg.generate_proof(&node.polynomial, &points);
    ...
    pub fn batch_proof_verify(
        ...
        let mut commitments_vector: Vec<F> = tree_proofs
            .iter()
            .map(|proof_node| {
        ...
        tree_proofs.par_iter().all(|proof_node| {
            if let Some(node) = proof_node {
                ...
                let b2 = node
                    .point
                    .par_iter()
                    .all(|point| commitments_vector.iter().any(|n| *n == point.1));
                b1 & b2
            } else {
                true
            }
