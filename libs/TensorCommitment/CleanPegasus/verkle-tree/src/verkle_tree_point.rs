...
impl VerkleTree {
    ...
    fn build_tree(
        ...
        datas: &Vec<Vec<u8>>,
        width: usize,
    ) -> Result<VerkleTree, VerkleTreeError> {
        if datas.is_empty() {
    ...
    */
        ...
        let proofs: Vec<Option<ProofNode>> = (0..index_for_proof.len())
            ...
            .map(|ind| {
                if index_for_proof[ind] != vec![] {
                    ...
                    let index_first_child: usize = if node.children.is_none() {
                        let index_first_child_data: usize = if width == 2 {
                    ...
                    let proof_of_node = self
                        .find_proof_node(
                            node,
                            index_for_proof[ind].clone(),
                            data,
                            index_first_child,
                        )
                        .expect("failed to generate proof for node");
                    Some(proof_of_node)
                } else {
                    None
                }
    ...
    fn find_proof_node(
        ...
        let proof =
            Proof::batch_new_aggregated(&self.pp, &node.commitment, &values, &indices_to_proof);
        match proof {
            Ok(proof) => {
    ...
    pub fn batch_proof_verify(
        root: Commitment,
        mut tree_proofs: Vec<Option<ProofNode>>,
        width: usize,
        indices: Vec<usize>,
        depth: usize,
        data: Vec<Vec<u8>>,
        ...
        let mut commitments_vector: Vec<Vec<u8>> = tree_proofs
            .iter()
            .map(|proof_node| {
        ...
        tree_proofs.par_iter().all(|proof_node| {
            if let Some(node) = proof_node {
                ...
                let b2 = node
                    .values
                    .par_iter()
                    .all(|point| commitments_vector.par_iter().any(|n| n == point));
                b1 & b2
            } else {
                true
            }
    ...
    pub fn root_commitment(&self) -> Option<Commitment> {
        ...
        self.root
            .as_ref()
            .map(|verkle_node| verkle_node.commitment.clone())
    }
}
