...
# Script directory for local paths
...
def get_model_sort_key(model_name):
    """
    Return a sort key for model ordering:
    1. LLaMA (7B, 13B, 30B, 65B) - increasing params
    2. LLaMA-2 (7B, 13B, 70B) - increasing params
    3. OPT-125M at the end
    """
    ...
    # Try to match model name to our ordering
    for key, order in model_order.items():
        if key in name_lower:
            return order
    
    # Unknown models go at the end
    return (99, 0)
...
def find_metric_files(data_dir, metric_name):
    ...
    # Sort by custom model order: LLaMA (increasing), LLaMA-2 (increasing), OPT
    metric_files.sort(key=lambda x: get_model_sort_key(x['model_name']))
...
def plot_multiple_models_from_data_dir(data_dir, metric_name, output_file, 
    ...
    if use_raw_metrics:
        ylabel = 'Error Probability (Raw ESD Metrics, Inverted)'
    elif normalize:
        ylabel = 'Error Probability (Normalized)'
    else:
        ylabel = 'Error Probability (Raw)'
    ax.set_ylabel(ylabel, fontsize=16, fontweight='bold')
    ax.set_xlabel('Network Quarters', fontsize=16, fontweight='bold')
    ax.grid(True, alpha=0.3, axis='y')
    
    # Add title above everything
    fig.suptitle(f'Layer Error Probability Comparison by Quarters\n(Metric: {metric_name})', 
